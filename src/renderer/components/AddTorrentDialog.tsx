...
interface AddTorrentDialogProps {
  ...
  onClose: () => void;
  onSubmit: (payload: AddTorrentPayload) => Promise<void>;
}
    ...
    <div className="modal-backdrop" role="presentation">
      <section className="modal" role="dialog" aria-modal="true" aria-label="Add torrent">
        <header className="modal-header">
          <h2>Add torrent</h2>
          <button type="button" className="icon-button" title="Close" onClick={onClose} disabled={busy}>
        ...
        <div className="modal-body">
          <label className="stacked-field">
            ...
            <div className="source-row">
              ...
              <input value={source} onChange={(event) => setSource(event.target.value)} disabled={busy || Boolean(file)} />
              <button type="button" className="command-button" onClick={chooseFile} disabled={busy}>
          ...
          <label className="stacked-field">
            ...
            <div className="source-row">
              ...
              <input value={downloadDir} onChange={(event) => setDownloadDir(event.target.value)} disabled={busy} />
            </div>
          </label>

          <div className="modal-options">
            <label>
              <input type="checkbox" checked={startNow} onChange={(event) => setStartNow(event.target.checked)} disabled={busy} />
              Start torrent
            </label>
            <label>
              <span>Peer limit</span>
              <input type="number" min={0} value={peerLimit} onChange={(event) => setPeerLimit(event.target.value)} disabled={busy} />
            </label>
          </div>
        ...
        <footer className="modal-footer">
          <button type="button" className="command-button" onClick={onClose} disabled={busy}>
            Cancel
          </button>
          <button type="button" className="command-button primary" onClick={submit} disabled={busy}>
            Add
          </button>
        </footer>
      </section>
